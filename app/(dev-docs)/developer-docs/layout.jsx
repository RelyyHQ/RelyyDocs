...
const banner = <Banner storageKey="developer-docs-banner" dismissible={false}>Developer Documentation</Banner>

const navbar = <RelyyDocsNavbar sectionLabel="Developer Docs" />
...
export default async function DeveloperDocsLayout({ children }) {
  const rootPageMap = await getPageMap('/').catch(() => [])
  const scopedPageMap = getScopedPageMap(rootPageMap, '/developer-docs')
